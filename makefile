...
temp/%.s: %.c $(SOURCE_HEADERS)
	$(MAIN_COMPILER) -Os -Oi $< --add-source --include-dir ./tools/cc65/include -o $(patsubst %.o, %.s, $@)
...
rom/$(ROM_NAME).nes: temp/crt0.o $(SOURCE_O)
	$(MAIN_LINKER) -C $(CONFIG_FILE) --dbgfile rom/$(ROM_NAME).dbg -o rom/$(ROM_NAME).nes temp/*.o tools/neslib_famitracker/runtime.lib -Ln rom/$(ROM_NAME).labels.txt
...
clean:
	-cmd /c rm -f rom/*.nes
	-cmd /c rm -rf temp/levels
	-cmd /c rm -f temp/*
	-cmd /c rm -f sounds/sfx/generated/*.s
	-cmd /c rm -f graphics/generated/*.png
	-cmd /c (cd temp & mkdir levels)
	-cmd /c (cd temp & touch empty)
	-cmd /c (cd temp & cd levels & touch empty)
