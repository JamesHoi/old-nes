...
void draw_win_screen(void) {
    ...
    if(killcnt==44 && found_key){
        put_str(NTADR_A(7, 2), "- Congratulations -");
        put_str(NTADR_A(4, 8), "You did the thing, and");
        put_str(NTADR_A(4, 9), "thus won the game!");
        put_str(NTADR_A(4, 13), "Now, please input");
        put_str(NTADR_A(4, 14), "your flag:");
    }else{
        put_str(NTADR_A(11, 4), "- Fail -");
        put_str(NTADR_A(4, 8), "You have to finish all");
        put_str(NTADR_A(4, 10), "the missions in the pause");
        put_str(NTADR_A(4, 12), "screen first.");
    }
